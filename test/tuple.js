...
describe("Tuple", function () {
  ...
  it("Should perform a tuple return that's sliced before being fed to another function (first var)", async () => {
    const commands = [
      [multiReturn, "intTuple", "0x41ffffffffffff", "0x00"],
    ...
    await expect(tx).to.emit(multiReturn.attach(multiReturn.address), "Calculated").withArgs(0xbad)
  ...
  it("Should perform a tuple return that's sliced before being fed to another function (second var)", async () => {
    ...
    await expect(tx).to.emit(multiReturn.attach(multiReturn.address), "Calculated").withArgs(0xdeed)
